...
from games.toeic_quiz_bank import TOEIC_QUESTION_BANK
...
class ToeicRushGame:
    game_type = "toeic_rush"
    title = "TOEIC Rush"
    subtitle = "TOEICの頻出語彙と文法を、早押し対戦でテンポよく練習するゲーム"
    category = "original"
    min_players = 1
    ...
    def __init__(self) -> None:
        self.players: Dict[str, ToeicRushPlayer] = {}
        self.settings = {
            "category": "mixed",
        ...
        self.message = "出題分野と問題数を決めて、TOEICの早押し対戦を始めましょう。"
        self.winner_text = ""
    ...
    def set_player_name(self, symbol: str, name: str) -> None:
        ...
        self.players[symbol] = ToeicRushPlayer(
            symbol=symbol,
            name=cleaned[:24],
            connected=connected,
    ...
    def reset_for_rematch(self) -> None:
        ...
        for symbol, (name, connected) in saved.items():
            self.players[symbol] = ToeicRushPlayer(symbol=symbol, name=name, connected=connected)
    ...
    def update_settings(self, settings: dict) -> None:
        ...
        if category not in {"mixed", "vocabulary", "grammar", "business"}:
    ...
    def submit_choice(self, symbol: str, card_index: Optional[int]) -> None:
        ...
        if correct:
            ...
            self.message = f"{player.name} が正解。{gain} 点獲得。"
            self.history.append(
    ...
    def resign(self, symbol: str) -> None:
        ...
        if len(self.joined_symbols()) < self.min_players:
            ...
            self.game_over = False
            self.phase = "waiting"
            self.message = "人数が足りなくなったため、待機中に戻りました。"
            return
    ...
    def _advance_to_next_question(self) -> None:
        ...
        label = label_map.get(self.current_question["category"], "TOEIC")
